...
# this code runs a script that is in the repository
# This can be used to change to your ggplot theme etc
# .. indicates jumping up one level from this folder, the rest shows the file path
```
...
# Own syntax, really likes lists
# More control over tables, probably the best for something like a thesis. 
```
...
Note that the default here is median and IQR for summary statistics; not what we might want. You can change the default behavior by setting a theme, or define manually (I prefer manually for complete control). Its more customary to use mean and SD (or SE) to summarize groups since we are most likely going to use parametric statistics for comparison.
...
table_Draco1 <- draco_ecomorph %>%
  ...
  tbl_summary(
    by = sex, # Creates the columns by sex
    missing = "no",
  ...
  modify_caption("Table 1. *Draco bimaculatus* Morphological Characteristics") %>% # Adding a table header
  modify_header(
    update = list(
      label ~ '',
      stat_1 ~ '**Female**', #is markdown **bold** formatting, markdown is default formatting for this
      stat_2 ~ '**Male**', # **Bold**, *italics*, may also be able to use underscores 
      p.value ~ '**P-value**')
  )

#show_header_names(table_Draco) 
#lets you preview code for changing header names and formatting  
# Note, every journal has different formatting rules, make sure your table compllies 
...
table_Draco2 <- draco_ecomorph %>%
  ...
  add_p(pvalue_fun = ~ style_pvalue(.x, digits = 2)) %>% #number of digits displayed for p-values
  # pvalue_fun sets the function for the p value 
...
# COnverted this to a flex table which allows it to be saved as a docx
```
...
#categorical and continous predictor 
...
# some journals could want test statistic included 
...
ex4 %>%
  as_flex_table() %>%
  flextable::save_as_docx(path = "../figs/ex4_flextable.docx") #saving the next table as a docx
```
